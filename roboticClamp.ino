...
#define BUTTON_SAVE_PIN 25 // Pin for saving the position
#define BUTTON_RESTORE_PIN 26 // Pin for restoring the position
...
const unsigned long AUTO_MODE_HOLD_DURATION = 2000; // Duration in milliseconds to hold the sensor for auto mode
bool autoMode = false; // Variable to track if the arm is in auto mode
unsigned long sensorPressStart = 0; // Variable to track when the sensor was pressed

const unsigned long HOLD_DURATION = 1000; // Duration in milliseconds to hold the button
...
void IRAM_ATTR handleSensorPress() {
  autoMode = false;
  autoModeTemp = false;
  Serial.print("automode -> false.");
  // Detach the interrupt if auto mode is not active
...
void loop() {
  ...
  if (armActive && autoMode) {
    // Attach the interrupt if it's not already attached
